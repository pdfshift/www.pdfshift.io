    ...
    <div>
        <NuxtLayout page="default">
            <main class="max-w-3xl mx-auto px-4 pb-12">
                <NuxtLink :to="`/sdks/${language}`" class="text-navy-700 inline-flex items-center gap-2">
                ...
                <aside class="mt-8 lg:mt-16 pb-16 lg:pb-32" v-if="related.length > 0">
                    ...
                    <ul class="mt-4 list-disc list-inside">
                        <li v-for="article in related" :key="article._id" class="my-4">
                            <NuxtLink :to="`/sdks/${language.toLowerCase()}/${clearUrl(article.library)}`" class="mt-4 text-xl hover:underline hover:text-purple" itemprop="name headline">
...
const { data } = await useAsyncData(route.fullPath, () => queryContent('sdks', language.value, library.value).findOne())
...
const { data: relatedEntries } = await useAsyncData(`${route.fullPath}-related`, () => queryContent('sdks', language.value).only(['_id', '_path', 'language', 'library']).find())
